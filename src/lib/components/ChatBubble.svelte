			...
			{#if isToolResult}
				...
				<details class="group">
					<summary class="cursor-pointer text-xs font-mono opacity-80">
						<Icon name="wrench" class="w-3.5 h-3.5 inline-block align-text-bottom" /> {entry.message?.toolName || 'tool'}
						...
						<details class="mt-2 -mx-1 rounded bg-base-300/30">
							<summary class="cursor-pointer text-xs py-1 px-2 flex items-center gap-1.5 min-w-0">
								<span class="flex-shrink-0">
									{#if name === 'bash'}<Icon name="bolt" class="w-3.5 h-3.5" />{:else if name === 'read'}<Icon name="document" class="w-3.5 h-3.5" />{:else if name === 'edit'}<Icon name="pencil" class="w-3.5 h-3.5" />{:else if name === 'write'}<Icon name="save" class="w-3.5 h-3.5" />{:else if name === 'lsp'}<Icon name="search" class="w-3.5 h-3.5" />{:else}<Icon name="wrench" class="w-3.5 h-3.5" />{/if}
								</span>
